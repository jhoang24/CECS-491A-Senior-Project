...
async function register(userInfo){
    ...
    const user = {
        ...
        firstname: firstname,
        lastname: lastname,
        verified: [false, verificationCode.toString()]
    }
    ...
    try {
      ...
      // Now you can return an error response if desired.
...
async function sendEmail(userInfo){
    ...
    const dynamoEmail = await getEmail(email);
    if (!dynamoEmail) {
        return util.buildResponse(403, { message: 'No account with that email' });
    }
  ...
  const mailOptions = {
      ...
      html: `<p>Your Verification Code is: ${dynamoEmail.verified[1]}</p><p>You can click this URL to confirm: <a href="${url}">Verify</a></p>`,
    };
